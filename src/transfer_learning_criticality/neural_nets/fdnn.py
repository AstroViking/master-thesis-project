...
class FDNN(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_layer_width: int, num_hidden_layers: int, num_classes: int, init_weight_mean: float=0, init_weight_std: float =1, init_bias_mean: float=0, init_bias_std: float=1):
    ...
    def forward(self, x: torch.Tensor, return_hidden_layer_activities: bool=False) -> Union[torch.Tensor, Tuple[torch.Tensor, NDArray]]:
        ...
        if return_hidden_layer_activities:
            ...
            hidden_layer_activities[0, :] = out.cpu().numpy()
        ...
        for i, layer in enumerate(self.hidden_layers):
            ...
            if return_hidden_layer_activities:
                hidden_layer_activities[i + 1, :] = out.cpu().numpy()
